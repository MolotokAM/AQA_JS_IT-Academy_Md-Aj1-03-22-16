...
describe('check Add-functionality', function () {
    test.each([[1, 2, 3], [30, 8, 38], [40, 10, 50]])('check addition of positive numbers: %d + %d = %d',
        async (a, b, c) => {
            expect(calc.add(a, b)).toBe(c);
        });

    test.each([[-1, 2, 1], [3, -3, 0], [-40, -10, -50]])('check addition of negative numbers: %d + %d = %d',
        async (a, b, c) => {
            expect(calc.add(a, b)).toBe(c);
        });

    test.each([[0, 2, 2], [3, 0, 3], [0, 0, 0]])('check addition of  "zero": %d + %d = %d',
        async (a, b, c) => {
            expect(calc.add(a, b)).toBe(c);
        });
    ...
    test('check result is "truthy"', async () => {
